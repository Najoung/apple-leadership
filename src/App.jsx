...
const topApple = (scores) =>
  [...APPLES].sort((a,b)=>(scores[b.id]||0)-(scores[a.id]||0))[0];

// ── Storage helpers ───────────────────────────────────────────────────────────
const DB = {
  async setSession(code, data) {
    try { await window.storage.set(`rl_sess_${code}`, JSON.stringify(data), true); return true; }
    catch { return false; }
  },
  async getSession(code) {
    try { const r = await window.storage.get(`rl_sess_${code}`, true); return r?JSON.parse(r.value):null; }
    catch { return null; }
  },
  async saveResult(code, name, data) {
    const key = `rl_res_${code}_${safe(name)}_${Date.now()}`;
    try { await window.storage.set(key, JSON.stringify({...data, name, ts:Date.now()}), true); return true; }
    catch { return false; }
  },
  async getResults(code) {
    try {
      const list = await window.storage.list(`rl_res_${code}_`, true);
      ...
      return all.filter(Boolean).sort((a,b)=>b.ts-a.ts);
    } catch { return []; }
  },
};
...
export default function App() {
  ...
  const [sess, setSess]         = useState(null); // {sessionName, teams, createdAt}
  ...
  // diagnosis flow
  const [pInfo, setPInfo]       = useState(null); // {name, team}
  ...
  if (sc==="home") return (
    <Page>
      <Box center>
        ...
        <p style={{...S.muted, margin:"0 0 28px", textAlign:"center", lineHeight:1.8}}>
          강사와 참여자가 실시간으로 연결되어<br/>
          팀별·부서별 리더십 분포를 한눈에 분석합니다
        </p>
        <Btn onClick={()=>setSc("f_create")} style={{marginBottom:12}}>
          👨‍🏫 강사 모드 — 세션 만들기
        </Btn>
        <Btn blue onClick={()=>setSc("p_join")}>
          🙋 참여자 모드 — 진단 참여하기
        </Btn>
      </Box>
    </Page>
      ...
      <Box>
        ...
        {newCode && <>
          <div style={S.codeBox}>
            ...
            <p style={{color:"#aaa",fontSize:"0.72rem",margin:"8px 0 0"}}>
              참여자 화면 → 참여자 모드 → 코드 입력
            </p>
          </div>
        ...
        <div style={{maxWidth:620,width:"100%"}}>
          ...
          <div style={S.dashHead}>
            ...
            <div style={S.dashStat}>
              <div style={S.dashBigNum}>{total}</div>
              <div style={{fontSize:"0.7rem",color:"#aaa"}}>참여자</div>
            </div>
          </div>
          ...
          <DashCard title="📊 유형별 분포">
            <div style={{display:"grid",gridTemplateColumns:"repeat(8,1fr)",gap:6,alignItems:"flex-end",paddingBottom:4}}>
              {APPLES.map(a=>{
                const cnt=dist[a.id];
                const h=cnt>0?Math.max((cnt/maxCnt)*90,18):6;
                return (
          ...
          <DashCard title={`👥 참여자 목록 (${total}명)`}>
            {total===0
              ...
              : <div style={{display:"flex",flexDirection:"column",gap:6}}>
                  {dashData.map((r,i)=>{
                    const a=APPLES.find(x=>x.id===r.topTypeId);
                    return (
            ...
            <DashCard title="🏢 팀별 사과 분포">
              {teams.map(tm=>{
                ...
                if(!members.length) return (
                  <div key={tm} style={{marginBottom:10,paddingBottom:10,borderBottom:"1px solid #1e1e3a"}}>
                    <span style={{fontSize:"0.78rem",color:"#555"}}>{tm} · 아직 참여자 없음</span>
                  </div>
                );
                return (
                  <div key={tm} style={{marginBottom:14}}>
                    ...
                    <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                      {members.map((m,i)=>{
                        const a=APPLES.find(x=>x.id===m.topTypeId);
                        return (
                          <div key={i} style={{background:a?.color+"25",border:`1px solid ${a?.color}55`,borderRadius:8,padding:"4px 10px",fontSize:"0.72rem",color:a?.color,fontWeight:600}}>
                            {a?.emoji} {m.name}
                          </div>
                        );
                      })}
          ...
          {missing.length>0 && total>=3 && (
            <DashCard title="⚠️ 우리 팀에 없는 사과" warn>
              {missing.map(a=>(
                <div key={a.id} style={{borderLeft:`3px solid ${a.color}`,paddingLeft:12,marginBottom:12}}>
          ...
          <button style={{...S.ghostDark,marginTop:4}} onClick={resetAll}>← 처음으로</button>
        </div>
      </Page>
    );
  }
    ...
    <Page>
      <Box>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:20}}>
          {sess?.teams?.map(tm=>(
            <button key={tm} onClick={()=>setTeamIn(tm)}
              style={{...S.teamBtn, borderColor:teamIn===tm?"#c0392b":"#e0e0e0", background:teamIn===tm?"#fff5f5":"#fff", color:teamIn===tm?"#c0392b":"#555", fontWeight:teamIn===tm?700:400}}>
              {teamIn===tm?"✅ ":""}{tm}
            </button>
          ))}
        </div>
        <Btn onClick={()=>{ setPInfo({name:nameIn.trim(),team:teamIn}); setSc("p_story"); }}>
          선택 완료 →
        </Btn>
      </Box>
    </Page>
      ...
      <Page bg={a.bg}>
        <Box>
          ...
          <div style={{textAlign:"center",marginBottom:14}}>
            <span style={{background:a.color,color:"#fff",borderRadius:50,padding:"4px 16px",fontSize:"0.78rem",fontWeight:700,display:"inline-block"}}>{a.type}</span>
          </div>
    ...
    <Page>
      <div style={{maxWidth:480,width:"100%"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:20}}>
          {APPLES.map(a=>(
            <button key={a.id} onClick={()=>setIntuitive(a.id)}
              style={{border:`2px solid ${a.color}`,borderRadius:14,padding:"14px 8px",
                background:intuitive===a.id?a.color:"#fff", color:intuitive===a.id?"#fff":a.color,
                cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:4,
                transition:"all 0.2s",fontFamily:"inherit",transform:intuitive===a.id?"scale(1.04)":"scale(1)",
                boxShadow:intuitive===a.id?`0 6px 20px ${a.color}55`:"none"}}>
              <span style={{fontSize:"1.6rem"}}>{a.emoji}</span>
              <span style={{fontSize:"0.72rem",fontWeight:700,textAlign:"center",lineHeight:1.3}}>{a.name}</span>
              <span style={{fontSize:"0.65rem",opacity:0.85}}>{a.type}</span>
            </button>
          ))}
        </div>
        <Btn style={{opacity:intuitive?1:0.4}} disabled={!intuitive} onClick={()=>setSc("p_diag")}>
          행동 진단 시작 →
        </Btn>
      </div>
    </Page>
    ...
    <Page bg={curA.bg}>
      <Box>
        <div style={{marginBottom:22}}>
          <div style={{display:"flex",justifyContent:"space-between",fontSize:"0.75rem",color:"#999",marginBottom:6}}>
            <span>{curA.name}</span>
            <span>{doneQ+1} / {APPLES.length*3}</span>
          </div>
          <div style={{height:6,background:"#e8e8e8",borderRadius:3,overflow:"hidden"}}>
            <div style={{height:"100%",width:`${pct}%`,background:curA.color,borderRadius:3,transition:"width 0.35s ease"}}/>
          </div>
        </div>
        <div style={{marginBottom:20}}>
          <span style={{background:curA.color,color:"#fff",borderRadius:50,padding:"5px 16px",fontSize:"0.78rem",fontWeight:700,display:"inline-block"}}>{curA.emoji} {curA.type}</span>
        </div>
        <p style={{fontFamily:"Georgia,serif",fontSize:"1rem",color:"#1a1a1a",lineHeight:1.75,fontWeight:700,marginBottom:30}}>
          {curA.questions[qI]}
        </p>
        <div style={{display:"flex",gap:10,justifyContent:"center",marginBottom:10}}>
          {[1,2,3,4,5].map(v=>{
            const sel=answers[`${curA.id}_${qI}`]===v;
            return (
              <button key={v} onClick={()=>handleAnswer(v)}
                style={{width:52,height:52,borderRadius:"50%",border:`2px solid ${curA.color}`,
                  background:sel?curA.color:"#fff",color:sel?"#fff":curA.color,
                  fontSize:"1.1rem",fontWeight:700,cursor:"pointer",transition:"all 0.15s",
                  fontFamily:"inherit",boxShadow:sel?`0 4px 14px ${curA.color}55`:"none"}}>
                {v}
              </button>
            );
          })}
        </div>
        <div style={{display:"flex",justifyContent:"space-between",fontSize:"0.7rem",color:"#bbb",padding:"0 2px"}}>
        ...
        <Box>
              ...
              return (
                <div key={id} style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
                  ...
                  <div style={{flex:1,height:7,background:"#f0f0f0",borderRadius:4,overflow:"hidden"}}>
                    <div style={{height:"100%",width:`${(score/15)*100}%`,background:a?.color,borderRadius:4,transition:"width 0.8s ease"}}/>
                  </div>
                  <span style={{fontSize:"0.7rem",fontWeight:700,color:a?.color,width:28,textAlign:"right"}}>{score}점</span>
                </div>
              );
            })}
          </div>

          {/* Intuitive vs Behavior */}
          <div style={{background:"#fff",border:"1px solid #eee",borderRadius:16,padding:16,marginBottom:14}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}>
              <div style={{flex:1,border:`2px solid ${intA?.color}`,borderRadius:10,padding:"10px 6px",textAlign:"center"}}>
                <div style={{fontSize:"0.65rem",color:"#aaa"}}>직관 선택</div>
              ...
              <div style={{flex:1,border:`2px solid ${top?.color}`,borderRadius:10,padding:"10px 6px",textAlign:"center"}}>
                <div style={{fontSize:"0.65rem",color:"#aaa"}}>행동 진단</div>
            ...
            <p style={{background:"#fffbf0",borderRadius:8,padding:"10px 12px",fontSize:"0.78rem",color:"#555",lineHeight:1.65,margin:0}}>
              {match
                ? `✨ 두 결과가 일치해요! ${top?.type}이 나의 진짜 핵심 리더십입니다.`
                : `🔍 원하는 리더십(${intA?.type})과 현재 행동(${top?.type}) 사이에 간격이 있어요. 이 간격을 좁히려면 오늘 무엇을 할 수 있을까요?`}
            </p>
          </div>
          ...
          <div style={{background:"#f9f9f9",borderRadius:16,padding:16,marginBottom:14}}>
            <div style={{marginBottom:10}}>
              <span style={{fontSize:"0.75rem",fontWeight:700,color:"#555"}}>💪 강점</span>
              <p style={{fontSize:"0.83rem",color:"#444",margin:"4px 0 0",lineHeight:1.65}}>{top?.strength}</p>
            </div>
            <div>
              <span style={{fontSize:"0.75rem",fontWeight:700,color:"#555"}}>⚠️ 주의점</span>
...
function Page({children,bg="linear-gradient(160deg,#fff9f0,#ffe8d6)"}) {
  return (
    <div style={{minHeight:"100vh",background:bg,display:"flex",alignItems:"flex-start",justifyContent:"center",padding:"20px 16px 60px",fontFamily:"'Noto Sans KR',sans-serif"}}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Noto+Sans+KR:wght@400;500;700&display=swap'); *{box-sizing:border-box;} button{cursor:pointer;}`}</style>
      {children}
    </div>
  );
}
...
function DashCard({title,children,warn=false}) {
  return (
    <div style={{background:warn?"#1a0f00":"#0d0d22",border:`1px solid ${warn?"#3a2000":"#1e1e3a"}`,borderRadius:16,padding:20,marginBottom:14}}>
      <h3 style={{margin:"0 0 14px",fontSize:"0.88rem",color:warn?"#f39c12":"#aaa",fontWeight:700}}>{title}</h3>
      {children}
    </div>
  );
}
function FloatingApples() {
  const apples=["🍎","🍏","🍎","🍏","🍎"];
  return (
    <div style={{display:"flex",justifyContent:"center",gap:8,marginBottom:16,fontSize:"1.8rem"}}>
      {apples.map((e,i)=>(
        <span key={i} style={{display:"inline-block",animation:`float${i} ${2.5+i*0.3}s ease-in-out infinite`,animationDelay:`${i*0.4}s`}}>
          {e}
          <style>{`@keyframes float${i}{0%,100%{transform:translateY(0) rotate(0deg)}50%{transform:translateY(-10px) rotate(${i%2?8:-8}deg)}}`}</style>
        </span>
      ))}
    </div>
  );
...
function Btn({children,onClick,style={},blue=false,disabled=false}) {
  return (
    <button disabled={disabled} onClick={onClick}
      style={{background:blue?"linear-gradient(135deg,#2980b9,#3498db)":"linear-gradient(135deg,#c0392b,#e74c3c)",color:"#fff",border:"none",borderRadius:50,padding:"13px 24px",fontSize:"0.92rem",fontWeight:700,width:"100%",marginBottom:8,fontFamily:"inherit",boxShadow:blue?"0 4px 16px rgba(41,128,185,0.35)":"0 4px 16px rgba(192,57,43,0.35)",transition:"opacity 0.2s",...style}}>
      {children}
    </button>
  );
}
...
function Input({...props}) { return <input {...props} style={{width:"100%",border:"1.5px solid #e0e0e0",borderRadius:10,padding:"10px 14px",fontSize:"0.9rem",fontFamily:"inherit",outline:"none",marginBottom:4,...props.style}}/>; }
...
const S = {
  ...
  ghostDark: {background:"transparent",color:"#555",border:"1px solid #2a2a4a",borderRadius:50,padding:"11px 24px",fontSize:"0.84rem",width:"100%",fontFamily:"inherit"},
